...
//
// The contents of this file are copied from the VesselMover mod.  The original file
// is a modified version of SpawnVessel by nightingale (Contract Configurator)
// The license on this file is the MIT license:
//
...
namespace KSPGeoCaching
{
    ...
    static public class VesselRespawn
    {
    ...
    public class VesselSpawn : MonoBehaviour
    {
        ...
        public Vessel spawnedVessel = null;
...
#if false
        ProtoVessel newProtoVessel = null;
        Guid guid;
        public void ShowDistance(Guid guid)
        {
            Log.Info("ShowDistance, guid: " + guid);
            this.guid = guid;
        }
        void ShowDistanceRoutine()
        {
            foreach (var v in FlightGlobals.Vessels)
                if (v.id == guid)
                {
                    newProtoVessel = v.protoVessel;


                    double distance = Vector3d.Distance﻿(FlightGlobals.ActiveVessel.GetWorldPos3D(), v.GetWorldPos3D());
                    DrawShadowedMessage("Distance to spawned vessel (" + newProtoVessel.vesselName + "): " + distance.ToString("N1"));
                    Log.Info("Distance to spawned vessel: " + distance.ToString("N1"));
                }
        }
#endif
...
#if FILE_SELECTION
                StartCoroutine(StartVesselSpawnRoutine(fullPath));
#endif
            }
        }
#if FILE_SELECTION
        private IEnumerator StartVesselSpawnRoutine(string fullPath)
        {
            OnSelected(fullPath, CraftBrowserDialog.LoadType.Normal);
            return null;
            ...
            craftBrowser = CraftBrowserDialog.Spawn(EditorFacility.VAB, HighLogic.SaveFolder, OnSelected, OnCancelled, false);
        }
        ...
        internal void DrawShadowedMessage(string message)
        {
            Log.Info("DrawShadowedMessage: " + message);
        ...
        {
            Log.Info("SpawnCraftRoutine, craftUrl: " + craftUrl);
            yield return null;
            yield return null;
        ...
        private void SpawnVessel(VesselData vesselData, List<ProtoCrewMember> crewData = null)
        {
            ...
            else
            {
                ...
                ProtoCrewMember[] crewArray = new ProtoCrewMember[0];
